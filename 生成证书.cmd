...
echo. ---------------环境变量包括配置文件路径cnf\openssl.cnf，key、csr、crt文件生成路径cert-ca\certs----------------------
echo.
echo.
...
:mkca1
echo. 通常CA（根证书）只用生成一次，是否继续生成，若cert-ca文件夹下已存在ca文件，将报错退出
...
openssl req -new -x509 -days 3650 -keyout cert-ca\ca.key -out cert-ca\ca.crt -subj %casubj% -extensions v3_ca
IF %ERRORLEVEL% NEQ 0 ( echo. 生成失败，请检查配置，即将退出!

exit ) else ( echo. 生成成功，CA保存在cert-ca\cert-ca\ca.key、cert-ca\ca.crt；请确定ca信息是否正确
...
openssl ecparam -genkey -name secp384r1 -out cert-ca\certs\%certname%.key
echo. 若需修改ECC私钥强度，请直接修改脚本的secp384r1字段.

echo. ECC私钥保存在cert-ca\certs\%certname%.key
pause
goto goon
...
:RSAKEY
echo. 即将生成RSA私钥
openssl genrsa -out cert-ca\certs\%certname%.key 2048
echo. 若需修改RSA私钥强度，请直接修改脚本的2048字段.
echo. RSA私钥保存在cert-ca\certs\%certname%.key
pause
goto goon
...
openssl req -new   -key cert-ca\certs\%certname%.key -out cert-ca\certs\%certname%.csr -subj %crtsubj%
echo. 若需修改扩展信息，请直接修改脚本-subj后对应字段.
echo. 请求文件保存在cert-ca\certs\%certname%.csr
pause
...
openssl ca -in cert-ca\certs\%certname%.csr -out cert-ca\certs\%certname%.crt -cert cert-ca\ca.crt -keyfile cert-ca\ca.key -extensions v3_req -config cnf\%certname%.cnf -days 3650 
...
:OK
echo.
echo. 生成成功，用户证书保存在cert-ca\certs\%certname%.crt
...
ECHO. 错误
echo. 删除生成的文件
del cert-ca\certs\%certname%.key
del cert-ca\certs\%certname%.csr
